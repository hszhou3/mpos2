...
\begin{itemize}
  ...
  \item \textbf{Public-coin eligibility:} each validator $v\in\Val$ independently becomes eligible in slot $s$ with probability $p\cdot\stakefrac{v}$ (a public-coin surrogate for VRF sortition). \mnote{I think that for simplicity we can drop the $stakefrac{v}$ part, as we are not referring to the stake in the \ProjBase.}\hnote{done}
...
Let $\theta\in\{\mathsf{equal},\mathsf{stake}\}$ be a protocol parameter. Define $\contrib(d)=1$ if $\theta=\mathsf{equal}$ and $\contrib(d)=\stakefrac{\val(d)}$ if $\theta=\mathsf{stake}$.\mnote{This could be assumed "equal" for the \ProjIdeal.}\hnote{thanks! Addressed}
\end{definition}
...
\State \Broadcast($b$) \mnote{I don't think that we need to rebroadcast $b$ here.}\hnote{Addressed}
\end{algorithmic}
\end{algorithm}
...
\end{equation*}
We select the tip $\hat{T}$ with maximum score (breaking ties by the deterministic order $\prec$):\mnote{tip or set of tips of non-conflicting branches?}\hnote{Addressed}
...
\State $\hat{T}\gets \bot, \hat{W}\gets -\infty$\mnote{Weight cannot be negative so maybe "0" instead of negative infinity?}\hnote{Thanks. Already revised and fixed}
