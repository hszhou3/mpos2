...
\begin{itemize}
  ...
  \item \textbf{Fork-choice.} We score tips by summing (optionally stake-weighted) short-reference counts of \emph{descendants} within the last $w$ slots, anchored at a fixed block $a$ (Alg.~\ref{alg:fork-choice}). \mnote{Note: A stake-weighted fork-choice rule is good to have for generalization of the protocol. I will just note that in our "privacy" focused design we are not disclosing the value of the stake of the validator, we only bind the chances of winning to the stake. Therefore, counting the weight as the number of short-reference counts is optimal as the frequency of proposing a block is a function of stake, so the block/reference count is also a function of stake.}
...
\end{equation}
for some constant $\varepsilon>0$. The slack $\omega(\log\lambda)$ is used for concentration bounds. \mnote{Might be good remind what the $H$, and $\Delta$ mean. I had to look for $H$ ;)}
...
$P$ maintains a local DAG $G_s=(V_s,E_s)$ and a ledger state view induced by its \emph{preferred tip} \mnote{preferred set of tips?}. It also caches reachability summaries and a per-validator latest-block map.
...
\id,\val{=}P,\slot{=}s,\txs{=}T,\refs{=}R\cup\{\ell\},y,\pi,\sigma\rangle,
  ...
  For each received block $b'$: verify VRF proof and signature; fetch missing parents; check (i) window constraint for short refs, (ii) at most one long-ref, (iii) \mnote{assert?} no cycles, (iv) \mnote{validate?} $R_w$ is an antichain in $G_{s,w}$, and (v) \mnote{confirm?} no UTXO conflicts vs.\ ancestors \mnote{exist?}. Reject if any check fails; otherwise integrate and update summaries and $\Tips(G)$.
  ...
  \]
  set preferred tip $\hat T_s\in\arg\max W(T)$ (tie-break by $(\slot,\id)$), and expose the induced ledger order. \mnote{Similar to \ProjIdeal comments instead of a single preferred tip we should use a set of preferred tips for references, also I think that the tie breaking mechanism should be based on something that is not controlled by the node - like VRF output. However, in practice I think that by using the VRF we can only limit the impact of adversarial actions on the conflict resolution. The adversary can have multiple identities that he can use to generate multiple VRF outputs per-slot. At the same time, I think, due to the construction of the VRF we use, there is a slight disadvantage for the node to divide the stake. So it might be more complex to be exploited by the adversary.}

  \item \textbf{Conflict resolution via CCA (Alg.~\ref{alg:cca-resolve}, on demand).}
  When two incomparable heads $i,j$ imply conflicting states, $P$ computes $c=\CCA(i,j)$ and compares $\BranchW(i;c,w)$ vs.\ $\BranchW(j;c,w)$. The winner determines which subtree remains eligible for inclusion in the preferred branch. This decision is \emph{consistent} with the anchored fork-choice (the preferred tip must lie on the heavier branch past the CCA).\mnote{"the preferred set of tips must lie on the heaviest branches past the CCA"?}
\end{enumerate}
...
Let $S$ be the current ledger state. A multiset of transactions $T\subseteq \Tx$ is valid if $\Valid(S,T)=\textsf{true}$. Two transactions $x,y$ conflict (denoted $\Conflicts(x,y)$) if they cannot both appear in any valid ledger state (e.g., they double-spend a UTXO). For a set of blocks $H\subseteq V$, \mnote{Haven't we just used the $H$ as the set of honest nodes above? This might be confusing.} let $\txs(H)=\bigcup_{h\in H}\txs(h)$ be the union of their transactions. We say a set $T$ conflicts with $H$, written $\Conflicts(T,H)$, if any transaction in $T$ conflicts with any in $\txs(H)$, or any two transactions in $T$ conflict with each other.
...
\end{equation*}
We require $|\longrefs(b)|\le1$. If there is a long-ref, call it $\longref(b)\in\Id$, \mnote{To the reader it might not be clear that $\longrefs(b)$ is related to $\longref(b)$ as "longrefs(b)" renders "refsout(b)" in the text.} and it contributes zero weight in fork-choice. \mnote{Similar as for \ProjIdeal, we could consider shifting that window slightly so that adversarial nodes cannot refer to blocks that have no chance to propagate through the network. But the problem with that approach is that we have not simulated it. So I think we could leave it for the "future" or mark it as potential "optimization".}
...
\end{equation*} \mnote{We could simplify this similarly to the \ProjIdeal. However, here we could preserve the $stakefrac{v}$ part as this is what we are going to simulate.}
On seeing a block $b$ with $(y(b),\pi(b))$, nodes verify the VRF proof $$\VRFVerify(\pk(v);\allowbreak \domsep\|\Encode(\chainid)\|\Encode(\netid)\|\Encode(\slot(b)), y(b), \pi(b))$$ and a digital signature $\VerifySig(\pk(v); b, \sigma(b))$. If either check fails, the block is ignored. \mnote{Note: If the first check fails and the second holds then this could be used for verifiable slashing of the sender of an ineligible proposal. WDYT?}
...
\Comment{Sample transactions (e.g., subsample UTXO pool with entropy $\rho$)}\mnote{This looks a bit too complex for the base model IMHO. I think we could simply state that a node is selecting up to a fixed number of transactions from the mempool. I've made a comment in one of the above sections about this random sampling being an transaction-redundancy decreasing optimization, which we might save for later.}
...
\State $b \gets \langle \id,\,\val,\,\slot,\,\txs,\,\refs,\,y,\,\pi,\,\sigma\rangle$ where $\val=v$, $\slot=s$, $\txs=T$, and $\sigma=\Sign(\sk(v);\id\|\val\|\slot\|\txs\|\refs\|y\|\pi)$ \mnote{Nit: Maybe we could drop the $\val=v$, $\slot=s$, $\txs=T$ part and use $v,s,T$ directly? Also we could define how to derive the $id$. }
...
\If{$\exists r\in \refs(b)$ such that $r\notin V$}
  \State Request missing parent $r$; enqueue $b$; \Return \mnote{As in the \ProjIdeal, might be clearer to be explicit here so that we don't need to enqueue $b$ and can reject it if $r$ is not available in a bounded time.}
\EndIf
...
\EndIf
\If{$\exists r,r'\in \refs(b),\ r\neq r'$ such that $r\nparallel r'$ in $G$} \mnote{It looks to me that this is the single place we use this $\nparallel$ notation. Might be good to explain it.}
...
\State $\Broadcast(b)$ \mnote{No need to rebroadcast}
\end{algorithmic}
\end{algorithm}
...
\end{remark} \mnote{Good remark, I think, it can be also justify that one-slot delay might not be enough for honest nodes to see the block for the same or next slot, so only adversary could be able to see them. Which means that we could add a "delay" or shift that window back.}
...
\begin{algorithmic}[1]
\Require Anchor $a$, $\Tips(G)$, window $w$, stake mode $\theta\in\{\mathsf{equal},\mathsf{stake}\}$\mnote{Stake mode is not used, maybe simplify?}
\Ensure Preferred tip $\hat{T}$ \mnote{Set of?}
...
\Else \Return $\min\nolimits_{\prec}\{i,j\}$ \Comment{deterministic tie-break}\mnote{The same as for \ProjIdeal, the block id is under control of the adversary, we could use a different (adversary-independent) mechanism for conflict resolution.}
\EndIf
\end{algorithmic}
\end{algorithm}
...
\end{remark} \mnote{Shouldn't this be done after block finalization? Are we assuming that a conflicted branch cannot be reorg-ed? I'm thinking about a case when an adversary reveals a conflicting branch to invalidate the other branch. This could happen especially when the adversary can exploit the tie-breaking mechanism and inject conflicting transactions to break the liveness. Just speculating here...}
...
We prove the DAG properties and ledger properties for \ProjBase under~\eqref{eq:base-params}. We write $\lambda_h \triangleq \mathbb{E}[\#\text{honest eligible per slot}]=\Theta(pH|\Val|)$. \mnote{Maybe $lambda_H$ for consistency with $H$?}
...
\end{theorem} \mnote{Shouldn't it be scaled by the allowed number of references per block?}
...
\end{proof}\mnote{I think that my speculation about the exploitation of the conflict resolution mechanism to control the branching could also be applied here, as the adversary could create conflicts and control which branch is perceived as conflict-free and grows. WDYT?}
...
\noindent
\textbf{Parameter Choices:} A window on the order of tens of slots (e.g., $w\approx 30$) typically suffices to cover network latency for a moderate honest block rate. The parallelism factor $N_{\MCP}$ tunes how many transactions each block samples, balancing leaderless throughput with verifier load. \mnote{I think that $N_{\MCP}$ should control the number of parallel blocks of fixed number of transactions.}
