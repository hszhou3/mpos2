...
% ----------------------------------------------------------------
\begin{remark}[Practical finality and choice of $k$]
\label{rem:practical-finality}
The formal $(k,\varepsilon)$-finality notion (Def.~\ref{def:k-eps-finality}) maps to an operational
finality time $T_{\mathrm{final}} = k\cdot\tau_{\mathrm{slot}}$ in implementations.  The security
analysis of the practical protocol shows that one may choose $k=\Theta(w)$ to obtain exponentially
small reorg probability (Theorem~4.9 establishes persistence with $k=\Theta(w)$ under the model
assumptions).  Hence, in practice one sets the short-reference window $w$ (subject to $w\ge\Delta$)
and then picks $k$ proportional to $w$, adding a small $\Theta(\log(1/\varepsilon))$ term to
meet an explicit $\varepsilon$ target.

As an operational example, if $\tau_{\mathrm{slot}}=20\,$s and the designer targets $T_{\mathrm{final}}\approx 5\,$min,
then $k\approx 15$ (i.e., burying by 15 short-reference descendants).  Different slot durations and
risk-tolerances produce different $k$; the lemma in Section~\ref{sec:ideal} (Lemma~\ref{lem:quant-finality}) gives the
quantitative dependence on $(w,\Delta,H,\varepsilon)$ used to pick $k$ for a desired $\varepsilon$.
\end{remark}
