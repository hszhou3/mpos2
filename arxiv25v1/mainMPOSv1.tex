...
Chain-based consensus protocols, including Bitcoin~\cite{Bitcoin} and PoS variants like Ouroboros Praos~\cite{EC:DGKR18}, employ sequential block production: in each round or slot, a designated leader proposes a new block to extend the chain. This design simplifies certain proofs of safety, but introduces a fundamental scalability–security trade-off. The throughput must be throttled to ensure blocks propagate fully before the next one, or else forks become likely. If blocks are generated too quickly relative to network delay, honest nodes may see different chains, violating common-prefix, and undermining the security and liveness guarantees. Thus, to maintain safety, these protocols deliberately limit their block rate, resulting in higher latency and lower capacity.\mnote{Nit: This paragraph mostly repeats what was stated above in the introduction. I'm not claiming that's a bad thing but when I read it I had a slight "dejavu". Maybe we could reduce a bit the redundancy and move a couple sentences from the above introduction here?}

Furthermore, when network conditions deteriorate (large delays or partitions), even these limited-throughput protocols can fail to maintain consistency. Single-proposer systems are particularly fragile: any disruption in message delivery can cause honest nodes to fall out of sync, allowing adversaries to execute chain reorganizations or double-spend attacks.\mnote{Maybe add "undermining/reducing chain resilience"? We are referring to resilience in the next paragraph so it might be nice to have it anchored here or in the next sentence?} In summary, traditional chain protocols face a bottleneck: increasing throughput or asynchrony directly threatens security, forcing a cautious pace.
...
We introduce \Proj, a family of PoS consensus protocols that realize multi-proposer, DAG-based consensus with strong security and performance. \mnote{I'm missing a direct reference to the "latency-friendliness".} Our key contributions include:
...
where:
\begin{itemize}
  \item \mnote{$\id(b)$ is its unique identifier (a hash),}\hnote{Taken} 
...
In any window of $\ell$ slots after GST, honest work makes \emph{linear progress along some honest chain} in the DAG: at least $\tau_D\cdot \ell$ honest blocks created in that window end up on the past of an honest party’s fork-choice tip (i.e., on its preferred branch). \mnote{I'm still not certain if this the right definition. My understanding of a DAG based ledger is that it assumes a certain level of parallelism, that is, a node can build on top of many tips. Therefore, I don't think we can assume \emph{linear progress along some honest chain} as the \emph{some honest chain} assume linearity. However, We might need to define "linearization" of the DAG structure, which we could use to define a global \emph{order} of blocks that are included in the DAG structure. Then, we can use a linearized DAG structure to define the \emph{linear progress of some honest chain}. What do you think?}\hnote{Thanks for your comments; your suggestions have been taken (in the main branch)!} 
...
\end{tikzpicture}\mnote{Moved the $s$ and $s+\ell$ a bit down.}\hnote{These editorial comments about the security defintion related figures have been taken in the main branch now. thanks.} 
...
\end{tikzpicture}\mnote{Moved the $s$ and $s+\ell$ a bit down.}\hnote{Taken}
...
beneath its fork-choice tip removed. \mnote{For example, couldn't we instead of $V_t^P$ use $\chain_i^s$?}
...
%  \node[blue!60!black] at (62.5,2.95) {\scriptsize $\mathsf{Frontier}_{t_1}^P$};
  \node[blue!60!black] at (62.5,3.1) {\scriptsize $\mathsf{Frontier}_{t_1}^P$};
...
%  \node[blue!60!black] at (92.5,2.95) {\scriptsize $\mathsf{Frontier}_{t_2}^Q$};
  \node[blue!60!black] at (92.5,3.1) {\scriptsize $\mathsf{Frontier}_{t_2}^Q$};
  ...
  \draw[decorate,decoration={brace,amplitude=6pt}] (70,2.95) -- (55,2.95)
    node[midway,above=7pt] {\scriptsize remove last $k_D$};
  \draw[decorate,decoration={brace,amplitude=6pt}] (100,2.95) -- (85,2.95)
    node[midway,above=7pt] {\scriptsize remove last $k_D$};
...
\begin{tikzpicture}[x=0.16cm,y=0.6cm,>=Latex,thick]
  % Axis
  \draw[-{Latex}] (0,0) -- (103,0) node[below right] {slots};
...
rule to $V_t^P$.\mnote{Same here, couldn't we instead of $V_t^P$ use $\chain_i^s$?} We say $\mathrm{Persistence}[k]$ holds if for all pairs of honest
...
\end{definition} \mnote{Nit: This definition is using transactions for the $k$ parameter, however, it looks to me that it should also work if we use blocks instead. Do we need to use transactions in this context as the number of transactions can be a function of the number of blocks? IMHO, I think that it would be more natural to express the persistence as number of blocks rather than the number of transactions.}
...
$E_t^Q(\tx)$ denote the event that honest party $Q$'s ledger $L_t^Q$ \mnote{Similarly here, couldn't we instead of $L_t^Q$ use $\chain_i^s$?} at the end of slot $t$
...
Coupled with TB (to prevent tips from fragmenting indefinitely) \mnote{TB is introduced in the next section so maybe "Coupled with tip boundedness (defined below, to prevent tips from fragmenting indefinitely)..."}
...
\begin{itemize}
  ...
  \end{remark}\mnote{I'm lacking one thing here. Due to the parallel structure of the DAG we can assume that the same set of transactions will be included in multiple branches. That is, we should expect a natural redundancy of transaction, hence the liveness might be less of a problem in this design. At least until we introduce some mechanisms to reduce the redundancy. For example, we can have verifiable random selection of transactions from mempool, which would naturally reduce the transaction redundancy.}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Public-coin eligibility:} each validator $v\in\Val$ independently becomes eligible in slot $s$ with probability $p\cdot\stakefrac{v}$ (a public-coin surrogate for VRF sortition). \mnote{I think that for simplicity we can drop the $stakefrac{v}$ part, as we are not referring to the stake in the \ProjBase.}
...
\begin{enumerate}
  ...
  \item \textbf{Block reception and integration (Alg.~\ref{alg:ideal-reception}).} Throughout the slot and by its end, $P$ receives all blocks broadcast in slot $s$. For each block $b$ not yet in $V_s$, $P$ verifies well-formedness and acyclicity, inserts $b$ and edges $(r\to b)$ \mnote{Why this way and not the other?} for all $r\in\refs(b)$, updates $\Tips(G)$ and incremental reachability summaries, and triggers \ForkChoiceUpdate.
  \item \textbf{Fork choice (Alg.~\ref{alg:ideal-anchored-fc}).} After integrating all blocks for slot $s$, $P$ runs anchored fork choice: for anchor $a$ (genesis or last finalized checkpoint), it computes window-filtered scores $W(T)$ for each $T\in\Tips(G)$ and sets the preferred tip $\hat{T}_s$ to the maximizer \mnote{Might be worth adding something more about the "preferred tip". Maybe that it's the tip of the branch with the highest weight after inclusion of the block $b$? Nevertheless, I think that it should be a set of tips that can be used for references rather than a single tip.}. The \emph{ledger view} of $P$ at the end of slot $s$ is the linearization induced by the preferred branch to $\hat{T}_s$.
  \item \textbf{Conflict resolution (Alg.~\ref{alg:ideal-cca}).} If two incomparable blocks $i,j$ imply conflicting ledger states above some ancestor, $P$ computes $c=\CCA(i,j)$ and compares window-filtered branch weights $\BranchW(i;c,w)$ vs.\ $\BranchW(j;c,w)$ to decide the winner. This rule is \emph{consistent} with the anchored fork choice: the preferred tip must lie on the heavier branch as measured from their CCA. \mnote{Ok, this sheds a bit of light on the definition of the "preferred tip".}
\end{enumerate}
...
We reuse the global notation from Section~\ref{sec:model}. Validators $\Val$ have stakes $\stake(v)>0$, total $\StakeTot=\sum_{v}\stake(v)$, share $\stakefrac{v} = \stake(v)/\StakeTot$. Slots $\Slot\subseteq\mathbb{N}$ are totally ordered. Transactions $\Tx$ exist but have no constraints. Block identifiers $\Id$ are totally ordered by, e.g., lexicographic hash order $\preceq_{\mathrm{id}}$; we then extend to a total order on blocks by $(\slot,\id)$ lexicographically. For blocks $u,v$, write $u \prec v$ if either $\slot(u)<\slot(v)$ or they share slot and $\id(u)\prec_{\mathrm{id}}\id(v)$. \mnote{This is what is what I was thinking about while making the comment about missing "linearization" in the section 2.}

A block is $b=\langle \id(b),\,\val(b),\,\slot(b),\,\txs(b),\,\refs(b)\rangle$. Here $\val(b)\in\Val$ and $\slot(b)\in\Slot$ denote its creator and slot; $\txs(b)\subseteq\Tx$ are its transactions; and $\refs(b)\subseteq\Id$ is a set of parent block identifiers (an antichain by construction). The reference-DAG is $G=(V,E)$ where $V$ is the set of all blocks, and $(r\to b)\in E$ \mnote{OK, in here, the $\to$ makes more sense as it has the same direction as $\leadsto$ and means that there is a path from $r$ to $b$. I will just note that technically we are including references to past blocks so to be completely aligned with the "real" DAG the direction should be reverted. But it's a nit.} iff $r\in \refs(b)$. We require $G$ to be acyclic. We write $x\leadsto y$ if there is a directed path from block $x$ to $y$. Define:
\begin{equation*}
...
Fix total order $\prec$ on vertices by $(\slot,\id)$. Scan vertices of $G_{s,w}$ \mnote{this notation is introduced in the next definition, would good to move it here or extract it somewhere above.} in decreasing $\prec$, adding $v$ if $v$ is incomparable with all current elements; output is a \emph{maximal} antichain. All honest nodes use the same $\prec$; hence determinism.
\end{definition}
\begin{definition}[\OptionalLongRef]
Given current slot $s$ and window $w$, optionally choose a single $\ell\notin G_{s,w}$ for connectivity: prefer the last finalized checkpoint if known; otherwise the $\prec$-max ancestor of $\min_{\prec}R$ \mnote{What is $R$? Is this the long-ref?} with $\slot(\ell)\le s-w$. Long-refs never contribute to weight.
\end{definition} \mnote{This could be made slightly more readable.}
...
$H(\rho\|\mathrm{txid}(x))<\tfrac{1}{N_{\MCP}}\cdot 2^\kappa$, then filter to a maximal conflict-free subset. \mnote{This is the "redundancy-reduction-optimization" I was referring in section 2. Do you think we need this complexity? I agree, it increases the transaction-denominated throughput but it might not be necessary for the \ProjIdeal case.}
\end{definition}
...
Let $\theta\in\{\mathsf{equal},\mathsf{stake}\}$ be a protocol parameter. Define $\contrib(d)=1$ if $\theta=\mathsf{equal}$ and $\contrib(d)=\stakefrac{\val(d)}$ if $\theta=\mathsf{stake}$.\mnote{This could be assumed "equal" for the \ProjIdeal.}
\end{definition}
...
We assume authenticated broadcast: any message sent by an honest party is delivered to every honest party within $\Delta$ slots after GST. \mnote{and messages cannot be forged (the "authenticated" part of the definition)?}
\end{definition}
\begin{definition}[Anchor Selection for Fork-Choice]
Let $a$ be the last finalized checkpoint if one exists; otherwise the earliest block whose slot $\le s-w$ on the preferred branch, else genesis. \mnote{Are we using "checkpoints"? I have not yet seen that we relay on them in the \ProjIdeal, so maybe for simplicity we could drop them. To me "checkpoints" are a neat feature, but are more of a bootstrapping optimization mechanics. WDYT?}
\end{definition}

\subsubsection{Idealized Antichain Selection and Block Creation}
In \ProjIdeal, when a validator $v$ is eligible at slot $s$, it samples transactions and references all current tips (or equivalently, all blocks in a maximum antichain). \mnote{Are we assuming honesty or should we add that explicit?} The following algorithm captures this:
...
\If{$\Eligibility(v,s)=1$}
  ...
  \State $b \gets \langle \id,\,\val=v,\,\slot=s,\,\txs=T,\,\refs=A\rangle$ \mnote{add longrefs? While looking at it I think that we could just drop long refs for \ProjIdeal as we have "unbounded" refs that could also be used to handle the DAG connectivity.}
...
\If{$\exists r\in \refs(b)$ such that $r\notin V$} \mnote{similar for longrefs, bit if not met should be rejected?}
  \State Request missing parents $r$; enqueue $b$; \Return \mnote{I would make it more complete, like: request missing parent, if the parent is not retrieved on time, then reject $b$. Now, we are missing part of logic that handles "enqueued" $b$.}
\EndIf
\If{$\exists r\in \refs(b)$ such that $r \in \Desc^*(b)$} \textbf{reject} $b$ \EndIf \mnote{Shouldn't we disallow references to blocks for the same slot (\ProjIdeal) or a number of slots (\ProjBase)? I think, that this could help to mitigate a form of "attack" where an adversary creates multiple proposals for the same (or nearby) slots and cross-refers between them to gain more weight. We could assume that there is some minimal delay of network propagation (expressed in number of slots) that prevents honest nodes from seeing honest blocks. We can use that bound to define this restriction on the acceptable reference distance. WDYT?}
\State Insert $b$ into $G$: add vertex and edges $(r\to b)$ for $r\in\refs(b)$ \mnote{the same for longref?}
\State Update $\Tips(G)$ and reachability summaries
\State \ForkChoiceUpdate($G$) \Comment{Update fork-choice state}
\State \Broadcast($b$) \mnote{I don't think that we need to rebroadcast $b$ here.}
\end{algorithmic}
\end{algorithm}
...
\end{equation*}
We select the tip $\hat{T}$ with maximum score (breaking ties by the deterministic order $\prec$):\mnote{tip or set of tips of non-conflicting branches?}
...
\State $\hat{T}\gets \bot, \hat{W}\gets -\infty$\mnote{Weight cannot be negative so maybe "0" instead of negative infinity?}
...
\Else \Return $\min\nolimits_{\prec}\{i,j\}$ \Comment{deterministic tie-break}\mnote{The value of the block id is controllable by the block constructor (by transaction selection or ordering), which can be used by an adversary to control the outcome of the conflict resolution. I would rather use VRF output for this as it's not controllable by the adversary. WDYT?}
\EndIf
\end{algorithmic}
\end{algorithm}
