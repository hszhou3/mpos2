...
\title{
Cryptareon: Scalable and Resilient Multi-Proposer Consensus\mnote{Maybe "Cryptareon: Latency-Friendly Resilient Multi-Proposer Consensus?"}\hnote{Done}\footnote{We refer interested readers to Álvaro Castro-Castilla’s ``New Cryptarchia,'' available at \url{https://www.notion.so/nomos-tech/New-Cryptarchia-202261aa09df8026b31ad5e09c1a3fbb}.\mnote{This footnote should be removed}\hnote{Done}}
\\
...
\end{equation*}
\end{definition} \mnote{I'm wondering if this indeed should be linear... If we assume leaderless case then we can have multiple honest blocks attached to one or more tip. Or maybe this should be interpreted as a single path on a DAG?}\hnote{Addressed}
...
\end{definition} \mnote{This tip boundedness is referring to what I think is lacking in previous definitions -- the fact that you can have multiple tips that can grow in parallel. My concern is that set of previous definitions does not directly refer to a DAG structure as it presents only linearisation of it.}
...
continually collapse \mnote{Nit: Is "collapse" the right word? It sounds a bit too strong.} the frontier. In this sense, TB can be viewed as the DAG
