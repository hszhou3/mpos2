...
% --- THIS PART IS NEW: notation at-a-glance ---
\begin{table}[htp!]
\vspace{-.3cm}
\vspace{-.3cm}
...
$H$ & honest fraction of stake; adversary fraction $\alpha=1-H$ \\
$\Delta$ & post-GST network delay bound (in slots) \\
$w$ & fork-choice window size (short-reference horizon) \\
$k$ & persistence depth (no reorgs beyond $k$ after finality) \\
$f$ & production parallelism factor (Sec.~\ref{sec:eval}) \\
$\Tips(G)$ & set of current tips in DAG $G$; $\Anc^*(\cdot)$ transitive ancestors \\
$\CCA(i,j)$ & closest common ancestor of blocks $i,j$ (Sec.~\ref{sec:base}) \\
$\contrib(d)$ & per-block contribution weight (default $1$; optionally stake-weighted) \\\bottomrule
\end{tabular}
...
% -----------------------------
% Notation: stake & proposal rates
% -----------------------------
\paragraph{Stake, eligibility and rates.}
Let $|V|$ be the number of validators (or stake units) and let $p\in(0,1]$ denote the per-validator
per-slot eligibility probability. Denote by
\[
  H\in(0,1]\qquad\text{(honest stake fraction)},
\]
\[
  \alpha\in[0,1]\qquad\text{(adversarial stake fraction, typically }\alpha\le 1-H\text{)}.
\]
We define the expected numbers of proposals per slot
\[
  \lambda_h \;:=\; \mathbb{E}[\#\text{ honest proposals per slot}] \;=\; p\cdot H\cdot |V|,
\]  
\[
  \lambda_a \;:=\; \mathbb{E}[\#\text{ adversarial proposals per slot}] \;=\; p\cdot \alpha\cdot |V|.
\]
When convenient we sometimes express adversarial load as $\lambda_a=(\alpha/H)\,\lambda_h$.
\smallskip

\paragraph{Asymptotic notation.}
We write $\Theta(\cdot)$ in the standard asymptotic sense: $f(n)=\Theta(g(n))$ means $f$ and $g$
agree up to constant factors.  In particular, $\Theta(pH|V|)$ denotes a quantity proportional to
$pH|V|$ (within multiplicative constants).   
%We write $\Theta(pH)$ to denote an \emph{expression proportional to} $pH$ (for instance a per-validator rate), and the surrounding context should indicate whether we mean that per-validator quantity or the aggregate rate 
...
\end{itemize}
%The validator must ensure $\txs(b)$ is conflict-free (no double-spends) relative to the ledger state, and $\refs(b)\cup\{\longref(b)\}$ is an antichain in the DAG, to preserve consistency. Honest blocks are immediately gossiped to all nodes. 

The validator must ensure $\txs(b)$ is conflict-free (no double-spends) relative to the ledger state;
the set of \emph{short references} $\refsin(b)$ forms an antichain in the windowed sub-DAG $G_{s,w}$;\footnote{Note here  $\refsin(b)=\{r\in\refs(b): s-\mathrm{slot}(r)<w\}$ and $G_{s,w}$ is the DAG induced by slots $[s-w+1,s]$.}
and there is \emph{at most one} optional long reference $\longref(b)$ to a block with $\mathrm{slot}\le s-w$,
which serves connectivity only and must not create a cycle (long-refs carry zero fork-choice weight).


The union of all blocks and reference edges forms a directed acyclic graph (DAG) $G$. We write $u\leadsto v$ if there is a directed path from block $u$ to $v$ in $G$. For a block $b$, let $\Anc(b)=\{a: a\leadsto b\}$ and $\Desc(b)=\{d: b\leadsto d\}$, with $\Anc^*(b)=\Anc(b)\cup\{b\}$, $\Desc^*(b)=\Desc(b)\cup\{b\}$. Two blocks $u$ and $v$ are {\em incomparable} (denoted $u\parallel v$) if neither $u\leadsto v$ nor $v\leadsto u$.
...
\paragraph{Confirming descendants and weight.}
We say a block $d$ \emph{confirms} a block $b$ if there is a directed path
from $b$ to $d$ using only \emph{short references} (i.e., edges that lie
inside the current window). Long references are connectivity-only and carry
zero fork-choice weight; they do not contribute to confirmation.

\begin{lemma}[Necessity of TB for block-confirmation liveness]
\label{lem:TB-necessity}
If Tip-Boundedness (TB) does not hold, then there exists an admissible
adversarial schedule under which an honest block $h$ never acquires any
\emph{confirming} (short-ref) descendants. Consequently, $h$ is never
confirmed under our fork-choice rule, even though it remains valid and is
eventually gossiped to all nodes. In particular, without TB, block-confirmation
liveness (and hence transaction liveness under the standard re-broadcast
assumption) can fail.
\end{lemma}

\begin{proof}[Proof sketch]
Without TB the adversary can keep the tip set size $T_t$ unbounded by releasing
incomparable blocks that omit $h$ from their short references. Each honest
proposer can include at most $R$ short references, so the chance that $h$ is
selected at time $t$ is at most $R/T_t$. By forcing $T_t$ to grow (or to
re-grow) so that $\sum_t (R/T_t)<\infty$, with positive probability $h$ is
never short-referenced, hence never confirmed. When $h$ exits the window, it
may receive long references, but those do not contribute confirmation weight
by definition; thus $h$ remains unconfirmed. TB enforces a bounded tip set,
which in turn yields a uniform lower bound on selection probability and
restores eventual confirmation.
\end{proof}

\begin{remark}[Validity vs. confirmation]
A block lacking confirming descendants is still \emph{valid} and will be
eventually \emph{seen} by all nodes. Our lemma concerns \emph{confirmation}
under the fork-choice rule (short refs only), not validity. Long references
ensure connectivity but carry zero weight and thus do not help confirmation.
At the transaction layer, standard re-broadcast ensures that honest
transactions reappear in other blocks; TB is still necessary to guarantee
bounded-delay confirmation of some carrier block.
\end{remark}
...
adversarial schedules where: 
(i) some honest blocks never accumulate any short-ref confirming descendants (breaking block-confirmation liveness under our rule; validity and eventual visibility are unaffected), 
% (i) some honest blocks never accumulate any confirming descendants (breaking liveness, under the no-redundancy assumption), 
and (ii) two
