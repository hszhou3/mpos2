...
\end{tikzpicture}\mnote{Moved the $s$ and $s+\ell$ a bit down.}\hnote{Fixed}
...
beneath its fork-choice tip removed. \mnote{For example, couldn't we instead of $V_t^P$ use $\chain_i^s$?}\hnote{addressed}
...
rule to $V_t^P$.\mnote{Same here, couldn't we instead of $V_t^P$ use $\chain_i^s$?}\hnote{addressed} 
We say $\mathrm{Persistence}[k]$ holds if for all pairs of honest
...
$E_t^Q(\tx)$ denote the event that honest party $Q$'s ledger $L_t^Q$ \mnote{Similarly here, couldn't we instead of $L_t^Q$ use $\chain_i^s$?}\hnote{addressed} at the end of slot $t$
...
Coupled with TB (to prevent tips from fragmenting indefinitely) \mnote{TB is introduced in the next section so maybe "Coupled with tip boundedness (defined below, to prevent tips from fragmenting indefinitely)..."}\hnote{addressed}
