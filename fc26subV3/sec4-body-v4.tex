...
\section{\ProjBase: A Practical DAG-Based PoS Protocol}
\label{sec:base}
...
\noindent\textit{Proof sketch.} We couple the windowed, CCA-anchored dynamics to the idealized analysis, use VRF concentration, and bound the frontier width via TB.
...
\label{thm:base-TB}
Assume $w\ge \Delta$ and let $\lambda_h=\mathbb{E}[\#\text{honest-eligible per slot}]$. Then $\mathrm{TB}[\beta]$ holds with $\beta=O(\lambda_h\cdot \Delta)$ w.h.p. Then $\mathrm{TB}[\beta]$ holds with $\beta=O(\lambda_h\cdot \Delta)$ w.h.p.
\end{theorem}
\begin{proof}[Proof sketch]
...
\begin{theorem}[Ledger properties of \ProjBase]\label{thm:base-ledger-summary}
Under the parameter discipline $w \ge \Delta + \omega(\log \lambda)$ and $H>1/2+\varepsilon$,
\ProjBase{} satisfies Persistence and Liveness after GST with constant parameters (in~$\lambda$).
Formal statements and proofs are given in Appendix~\ref{app:base-security}.
\end{theorem}
